...
const deleteSupplier = (row) => {
    const config = {
        ...
        itemName: row.name,
        // Kirim URL-nya ke modal
        url: route("api.settings.deleteSupplier", { id: row.id }),
    };
    showConfirmModal.value.open(config);
};
const openForceDeleteModal = (row) => {
    const config = {
        title: "Hapus Permanen Supplier",
        message:
            "Data ini akan dihapus selamanya dan TIDAK BISA dikembalikan. Anda yakin ingin melanjutkan menghapus",
        itemName: row.name,
        url: route("api.settings.deleteSupplier", {
            id: row.id,
            permanen: true, // <-- Ini kuncinya
        }),
    };
    showConfirmModal.value.open(config);
};

const restoreSupplier = (row) => {
    isActionLoading.value = true; // Nyalakan loader

    router.put(
        route("api.settings.restoreSupplier", { id: row.id }),
        {},
        {
            preserveScroll: true,
            onSuccess: () => {
                refreshTable(); // Refresh tabel sampah
            },
            onError: (errors) => {
...
<template>
    ...
    />
    <DeleteConfirm ref="showConfirmModal" @success="refreshTable" />
    <div
    ...
    >
        ...
        <DataTable
            ref="datatable"
            :serverSide="true"
            :endpoint="route('api.settings.getSupplier')"
            :columns="columns"
        ...
        >
            ...
            <template #address="{ row }">
                <span class="text-sm text-gray-600 dark:text-gray-400">
                    {{ row.address ? row.address.substring(0, 40) : "-" }}
                    <span v-if="row.address && row.address.length > 40"
                        >...</span
                    >
                </span>
            </template>
            <template #type="{ row }">
                <span
                    class="px-2 py-1 text-xs font-medium capitalize rounded-full"
                    :class="{
                        'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300':
                            row.type === 'type_online',
                        'bg-purple-100 text-purple-800 dark:bg-purple-900 dark:text-purple-300':
                            row.type === 'type_offline',
                    }"
                >
                    {{ row.type === "type_online" ? "Online" : "Offline" }}
                </span>
                       
            </template>
            <template #status="{ row }">
                <span
                    ...
                    :class="{
                        'bg-lime-100 text-green-800 dark:bg-green-900 dark:text-green-300':
                            row.status === 'active',
            ...
            <template #aksi="{ row }">
                ...
                <button
                    class="px-2 py-1 ml-2 bg-red-500 rounded hover:bg-red-700"
                ...
                <div class="" v-else>
                    <button
                        @click="restoreSupplier(row)"
                        class="px-2 py-1 rounded bg-lime-500 hover:bg-lime-700"
                    ...
                    <button
                        @click="openForceDeleteModal(row)"
                        class="px-2 py-1 ml-2 bg-red-500 rounded hover:bg-red-700"
