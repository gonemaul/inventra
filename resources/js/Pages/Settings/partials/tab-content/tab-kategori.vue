...
const deleteCategory = (row) => {
    const config = {
        ...
        message: "Anda yakin ingin menghapus kategori",
        itemName: row.name,
        // Kirim URL-nya ke modal
        url: route("api.settings.deleteCategory", { id: row.id }),
    };
...
const openForceDeleteModal = (row) => {
    const config = {
        ...
        itemName: row.name,
        url: route("api.settings.deleteCategory", {
            id: row.id,
...
const restoreCategory = (row) => {
    isActionLoading.value = true; // Nyalakan loader

    router.put(
        route("api.settings.restoreCategory", { id: row.id }),
        {},
        {
            preserveScroll: true,
            onSuccess: () => {
    ...
    >
        ...
        >
            <div>
                ...
                >
                    Kelola semua kategori untuk produk.
                </p>
        ...
        <DataTable
            ref="datatable"
            :serverSide="true"
            :endpoint="route('api.settings.getCategory')"
            :columns="columns"
