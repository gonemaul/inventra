...
const deleteType = (row) => {
    const config = {
        ...
        message: "Anda yakin ingin menghapus tipe produk",
        itemName: row.name,
        // Kirim URL-nya ke modal
        url: route("api.settings.deleteType", { id: row.id }),
    };
...
const openForceDeleteModal = (row) => {
    const config = {
        ...
        itemName: row.name,
        url: route("api.settings.deleteType", {
            id: row.id,
...
const restoreType = (row) => {
    ...
    router.put(
        route("api.settings.restoreType", { id: row.id }),
        {},
        {
            preserveScroll: true,
            onSuccess: () => {
