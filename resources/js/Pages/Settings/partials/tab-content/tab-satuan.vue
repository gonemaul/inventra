...
const restoreUnit = (row) => {
    isActionLoading.value = true; // Nyalakan loader

    router.put(
        route("api.settings.restoreUnit", { id: row.id }),
        {},
        {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    ...
    >
        ...
        <DataTable
            ref="datatable"
            :serverSide="true"
            :endpoint="route('api.settings.getUnit')"
            :columns="columns"
