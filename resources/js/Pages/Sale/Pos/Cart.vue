        ...
        >
            ...
            <div v-if="cartStep === 1">
                <div
                    ...
                    :key="index"
                    class="relative flex gap-3 p-3 mb-3 bg-white border border-gray-100 shadow-sm dark:bg-gray-800 rounded-xl dark:border-gray-700 group hover:border-lime-300"
                >
                    <!-- 1. Product Image -->
                    <div class="shrink-0">
                        <img
                            ...
                            :src="item.image_url"
                            alt="Product"
                            class="w-14 h-14 object-cover rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50"
                        />
                        <div
                            v-else
                            class="w-14 h-14 flex items-center justify-center rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-400"
                        >
                    ...
                    <div class="flex-1 min-w-0 flex flex-col justify-between ml-1">
                        <!-- Top: Name & Remove -->
                        <div class="flex justify-between items-start gap-1">
                            <div>
                                <h4 class="text-xs font-bold text-gray-800 dark:text-gray-100 leading-snug line-clamp-2">
                                    {{ item.name }}
                                </h4>
                                <div class="flex items-center gap-1 mt-0.5 text-[10px] text-gray-500">
                        ...
                        <div class="flex items-end justify-between gap-2 mt-2">
                             <!-- Price Input / Display -->
                             <div class="flex-1">
                                <label v-if="!['Jasa', 'Layanan'].includes(item.category?.name)" class="text-[9px] font-bold text-gray-400 uppercase">Harga @</label>
                                <div class="relative group/price">
                                    <!-- Price Input For Non-Service -->
                                    <InputRupiah
                                        v-if="!['Jasa', 'Layanan'].includes(item.category?.name)"
                                    ...
                                    />
                                     ...
                                     <!-- Total & Toggle For Service -->
                                    <!-- Total & Toggle for Service (Hybrid) -->
                                    <div v-else class="flex items-end justify-between w-full gap-2">
                                             ...
                                             <button
                                                @click="() => {
                                                    ...
                                                    if (item.selling_price == 0) {
                                                        ...
                                                        item.original_price = item.selling_price; // Save current as original before zeroing
                                        ...
                                        <!-- Custom Price Input (Only show if NOT Free) -->
                                        <div v-if="item.selling_price != 0" class="relative w-32">
                                            <span class="absolute left-2 top-1.5 text-xs font-bold text-gray-400">Rp</span>
                                            <input
                                                type="number"
                                                v-model="item.selling_price"
                                                @input="recalcFromPrice(item)"
                                                class="w-full pl-7 pr-2 py-1 text-right text-sm font-bold border-gray-200 rounded-lg focus:ring-lime-500 focus:border-lime-500"
                                                placeholder="0"
                                            />
                                        </div>
                                        <!-- Text Gratis Label if Free -->
                                        <div v-else class="text-xs font-bold text-lime-600">
                                            Rp 0
                                        </div>
                                    </div>
                                </div>
                             ...
                             <!-- Qty Control (Hidden for Service) -->
                             <div v-if="!['Jasa', 'Layanan'].includes(item.category?.name)" class="flex items-center border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden shrink-0 h-8">
                                <button @click="updateQty(index, -1)" class="w-8 h-full flex items-center justify-center bg-gray-50 hover:bg-red-50 hover:text-red-500 active:bg-gray-200 transition text-lg font-bold">
                                    -
                                </button>
                                <input
                                    type="number"
                                    ...
                                    class="w-10 h-full text-center text-sm font-bold border-none bg-white dark:bg-gray-800 p-0 focus:ring-0"
                                />
                                <button @click="updateQty(index, 1)" class="w-8 h-full flex items-center justify-center bg-gray-50 hover:bg-lime-50 hover:text-lime-600 active:bg-gray-200 transition text-lg font-bold">
                                    +
                                </button>
                             </div>
                        </div>
